...
const { sslFor, generateCaseReference } = require(path.join(__dirname, "..", "dist", "index.js"));
...
const ADMISSIONS = {
  ...
  requirements: [
    ...
    { key: "transfer_certificate", label: "Transfer certificate", required: true },
    { key: "id_proof", label: "ID proof (Aadhaar or passport)", required: true },
    ...
    { key: "entrance_score", label: "Entrance exam score card", required: true, validityDays: 365 },
...
const DEMO_CASES = [
  // Business Loan
  { workflow: "business-loan", stage: "Follow up", name: "Meera Nair", organisation: "Nair Textiles",
    email: "meera.nair@example.com", phone: "+91 98450 11223",
    data: { loan_type: "SME Term Loan", loan_amount: 1800000, entity_type: "Proprietorship", company_name: "Nair Textiles" } },
  { workflow: "business-loan", stage: "PVT LTD Document Collection", name: "Arjun Mehta", organisation: "Mehta Agro Exports Pvt Ltd",
    email: "arjun.mehta@example.com", phone: "+91 98220 44556",
    data: { loan_type: "LAP", loan_amount: 5500000, entity_type: "Private Limited", company_name: "Mehta Agro Exports Pvt Ltd" } },
  { workflow: "business-loan", stage: "Pending", name: "Sunita Reddy", organisation: "Reddy Hardware",
    email: "sunita.reddy@example.com", phone: "+91 99630 77889",
    data: { loan_type: "SME Term Loan", loan_amount: 900000, entity_type: "Partnership", company_name: "Reddy Hardware" } },
  // College Admissions
  { workflow: "college-admissions", stage: "Documents pending", name: "Ananya Sharma", organisation: null,
    email: "ananya.sharma@example.com", phone: "+91 90040 12321",
    data: { course_applied: "B.Sc Computer Science", entrance_exam: "CUET", previous_school: "DAV Public School" } },
  { workflow: "college-admissions", stage: "New enquiry", name: "Rohan Verma", organisation: null,
    email: "rohan.verma@example.com", phone: "+91 98110 45654",
    data: { course_applied: "BBA", previous_school: "St. Xavier's" } },
  { workflow: "college-admissions", stage: "Under review", name: "Fatima Khan", organisation: null,
    email: "fatima.khan@example.com", phone: "+91 99870 78987",
    data: { course_applied: "M.Com", entrance_exam: "University entrance", previous_school: "Loyola College" } },
];
...
(async () => {
  const out = await sql.begin(async (tx) => {
    ...
    const summary = { tenant: tenants[0].name, workflowCreated: false, casesCreated: 0, casesSkipped: 0 };
    ...
    for (const spec of DEMO_CASES) {
      ...
      if (!created) throw new Error(`Could not allocate a reference for ${spec.email}`);
