...
export function useSelection(visibleIds: string[]) {
  ...
  const toggleAll = React.useCallback(() => {
    // Compare against the *visible* intersection, not raw prev.size: after a
    // filter shrinks the list, prev can still hold hidden ids and a naive
    // size check would refuse to deselect, re-selecting instead.
    setSelected((prev) => {
      const allowed = new Set(visibleIds);
      const visibleSelected = [...prev].filter((id) => allowed.has(id));
...
export function SelectCheckbox({
  ...
  return (
    <input
      ...
      className="h-4 w-4 cursor-pointer rounded-[6px] border-input accent-primary"
    />
  );
}
