...
export function ActivityTab({ events }: { events: ActivityEvent[] }) {
  return <Card className="gap-0 overflow-hidden py-0"><div className="border-b p-4"><div className="font-medium">Activity</div><p className="text-sm text-muted-foreground">Every touchpoint on this case, newest first.</p></div><div className="flex flex-col p-4">{events.map((event, index) => <div key={`${event.at}-${index}`} className="relative flex gap-3 pb-5 last:pb-0">{index < events.length - 1 ? <div className="absolute left-3.5 top-7 h-[calc(100%-1.25rem)] w-px bg-border" /> : null}<div className={`z-10 flex size-7 shrink-0 items-center justify-center rounded-full border ${event.failed ? "border-red-200 bg-red-50 text-red-600 dark:border-red-900 dark:bg-red-950 dark:text-red-400" : "border-transparent bg-muted text-muted-foreground"}`}><Icon name={event.icon} size={14} /></div><div className="min-w-0 flex-1 pt-0.5"><div className="text-sm">{event.title}</div>{event.detail ? <div className="mt-0.5 text-xs text-muted-foreground">{event.detail}</div> : null}<div className="mt-0.5 text-xs text-muted-foreground/70">{formatDateTime(event.at)}</div></div></div>)}</div></Card>;
}
